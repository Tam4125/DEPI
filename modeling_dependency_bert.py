...
class DependencyBertSelfAttetion(nn.Module):
    ...
    def forward(self, 
        ...
        print(f"context_layer: {context_layer.size()}")
...
class DependencyBertLayer(GradientCheckpointingLayer):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, 
        ...
        print(f"layer_output: {layer_output.size()}")
...
class DependencyBertEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        # Pairs of Original and Dependency Integrated Self Attention
        all_self_attn = () if output_attentions else None
        # Hidden states for all layer
        all_hidden_states = () if output_hidden_states else None

        for layer_module in self.layer:
            layer_outputs = layer_module(hidden_states=hidden_states,
                                         attention_mask=attention_mask,
                                         dependency_matrix=dependency_matrix,
                                         output_attentions=output_attentions,
                                        )
            ...
            if output_attentions:
                all_self_attn = all_self_attn + ([layer_outputs[1], layer_outputs[2]],)
